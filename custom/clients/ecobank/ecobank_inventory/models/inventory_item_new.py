...
class InventoryItem(models.Model):
            ...
            if all_additions:
                ...
                if len(sorted_receipts) == 1:
                    ...
                    if rec.valuation_method == 'fifo':
                        ...
                        quantities = 0.0
                        total_values = 0.0
                        for line in sorted_receipts:
                            quantities += line.quantity_remaining
                            total_values += (line.quantity_remaining * line.cost)
                        if quantities and total_values:
                            average_cost = total_values / quantities
                        current_value = average_cost * quantities
                    """
                    elif rec.valuation_method == 'average':
    ...
    def check_inventory_value_0(self, date):
        for rec in self:
            current_value = 0.0
            average_cost = 0.0
            ...
            if len(receipts) < 1:
                ...
                if len(sorted_receipts) == 1:
                    ...
                    if rec.valuation_method == 'fifo':
                        ...
                        second_qty = sorted_receipts[-1].quantity_remaining
                        second_value = sorted_receipts[-1].cost
                        average_cost = ((first_qty*first_value)+(second_qty*second_value))/(first_qty+second_qty)
                        """
                        current_value = average_cost * quantities
