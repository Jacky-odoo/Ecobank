...
class OdooService(win32serviceutil.ServiceFramework):
    ...
    def SvcStop(self):
        ...
        # Stop the running Byte  ERP: say it's a normal exit
        win32api.TerminateProcess(int(self.odooprocess._handle), 0)
        servicemanager.LogInfoMsg("Byte  ERP stopped correctly")

    def SvcDoRun(self):
        # We start Byte  ERP as an independent process, but we keep its handle
        ...
        servicemanager.LogInfoMsg('Byte  ERP up and running')
        # exit with same exit code as Byte  ERP process
