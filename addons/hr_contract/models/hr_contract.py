...
class ContractType(models.Model):
    ...
    is_consultant = fields.Boolean(default=False)
...
class Contract(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', required=True, track_visibility='onchange', readonly=True)
    department_id = fields.Many2one('hr.department', string="Department", readonly=True, track_visibility='onchange')
    type_id = fields.Many2one('hr.contract.type', string="Contract Type", track_visibility='onchange', required=True, default=lambda self: self.env['hr.contract.type'].search([], limit=1))
    ...
    wage = fields.Float('Basic Salary', digits=(16, 2), required=True, help="Basic Salary of the employee", track_visibility='onchange')
    ...
    active = fields.Boolean(default=True, string='Active', track_visibility='onchange')
    payment_option = fields.Selection([('weekly', 'Weekly'),
                                       ('bi-weekly', 'Bi-Weekly'),
                                       ('monthly', 'Monthly')],
                                      default='monthly', string="Payment Option", track_visibility='onchange')
