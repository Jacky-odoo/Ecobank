...
class HrAttendance(models.Model):
    ...
    def _cron_auto_check_out(self):
        no_check_out_attendances = self.env['hr.attendance'].search([
            ('check_out', '=', False), ('check_in', '<', fields.Datetime.to_string(datetime(datetime.today().year, datetime.today().month,
                                                                        datetime.today().day, 00, 00, 00)))])
        hour = 17
        min = 00
        checkout_time = self.env.user.company_id.checkout_time
        if checkout_time:
            hour = int(str(checkout_time).split(':')[0])
            min = int(str(checkout_time).split(':')[-1])

        if no_check_out_attendances:
            for att in no_check_out_attendances:
                if fields.Datetime.from_string(att.check_in) < datetime(datetime.today().year, datetime.today().month,
                                                                        datetime.today().day, 00, 00, 00):
                    if fields.Datetime.from_string(att.check_in) > datetime(fields.Datetime.from_string(att.check_in).year,
                                                                           fields.Datetime.from_string(
                                                                               att.check_in).month,
                                                                           fields.Datetime.from_string(
                                                                               att.check_in).day, hour, min, 00):
                        att.check_out = fields.Datetime.to_string(att.check_in + relativedelta(minutes=5))
                    if fields.Datetime.from_string(att.check_in) < datetime(fields.Datetime.from_string(att.check_in).year,
                                                                           fields.Datetime.from_string(
                                                                               att.check_in).month,
                                                                           fields.Datetime.from_string(
                                                                               att.check_in).day, hour, min, 00):
                        att.check_out = fields.Datetime.to_string(
                            datetime(fields.Datetime.from_string(att.check_in).year,
                                     fields.Datetime.from_string(
                                         att.check_in).month,
                                     fields.Datetime.from_string(
                                         att.check_in).day, hour, min, 00
