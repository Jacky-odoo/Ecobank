...
class Employee(models.Model):
    ...
    academic_qualification = fields.Text(string="Academic Qualification")
    name_related = fields.Char(related='resource_id.name', string="Resource Name", readonly=True, store=True)
    country_id = fields.Many2one('res.country', string='Nationality (Country)', track_visibility='onchange')
    birthday = fields.Date('Date of Birth',  track_visibility='onchange')
    ssnid = fields.Char('Nassit No', help='Social Security Number',  track_visibility='onchange')
    ...
    marital = fields.Selection([
        ...
        ('widower', 'Widower'),
        ('divorced', 'Divorced')
    ], string='Marital Status',  track_visibility='onchange')
    department_id = fields.Many2one('hr.department', string='Department', track_visibility='onchange')
    address_id = fields.Many2one('res.partner', string='Working Address', track_visibility='onchange')
    ...
    work_location = fields.Char(string='Work Location/Branch', track_visibility='onchange')
    ...
    employee_type = fields.Selection([('local', 'Local'),
                                      ('foreign', 'Foreign'),
                                      ], default='local', string="Employee Type")
    ...
    age = fields.Integer('Age', compute='_compute_age', readonly=True, store=True, track_visibility='onchange')

    @api.constrains('work_email')
    def check_email(self):
        if self.work_email:
            if not tools.single_email_re.match(self.work_email):
                raise ValidationError("Invalid Email Address Entered. Error Code BYT007")

    @api.constrains('birthday')
    def check_birthday(self):
        if fields.Date.to_string(date.today()) < self.birthday:
            raise fields.UserError("Birthday cannot be in the future")
