    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        if res.get('headers'):
            try:
                ...
                pass
        if self.project_id:
            current_objects = filter(None, headers.get('X-Byte  ERP-Objects', '').split(','))
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Byte  ERP-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Byte  ERP-Tags'] = ','.join(self.tag_ids.mapped('name'))
