...
class AccountAsset(models.Model):
    ...
    name = fields.Char(
        string='Asset Description', size=64, required=True,
        readonly=True, states={'draft': [('readonly', False)]})
    code = fields.Char(
        string='Co Asset No.', size=32, readonly=True,
    ...
    value_residual = fields.Float(
        compute='_compute_value_depreciation',
        digits=dp.get_precision('Account'),
    ...
    date_start = fields.Date(
        string='In Service Date', readonly=True,
        states={'draft': [('readonly', False)]},
        default=fields.Datetime.now,
        help="Date the asset go into service")
    date_end = fields.Date(
        string='Asset End Date', readonly=True,
        help="Date the asset will completely depreciate")
    acquired_date = fields.Date(
        string='Acquired Date', readonly=True,
        states={'draft': [('readonly', False)]},
        help="Date this Asset was acquired.")
    ...
    sys_no = fields.Char(string='Sys No',
                         index=True,
                         readonly=True)
    partner_id = fields.Many2one(
        comodel_name='account.asset.user', string='Assigned To', readonly=True,
        ondelete='restrict',
        states={'draft': [('readonly', False)]})
    ...
    method_number = fields.Integer(
        ...
        states={'draft': [('readonly', False)]},
        default=lambda self: self.profile_id.method_number,
        help="The number of years needed to depreciate your asset")
    method_number_month = fields.Integer(
        string='Number of Months', readonly=True,
        states={'draft': [('readonly', False)]},
        default=lambda self: self.profile_id.method_number_month,
        help="Number of Months")
    has_month = fields.Boolean(string="Include Months?",
                               default=lambda self: self.profile_id.has_month)
    ...
    image = fields.Binary("Logo", attachment=True,
                          help="This field holds the image of the asset, limited to 1024x1024px.")
    image_medium = fields.Binary("Medium-sized image", attachment=True,
                                 help="Medium-sized image of asset. It is automatically "
                                      "resized as a 128x128px image, with aspect ratio preserved.")
    image_small = fields.Binary("Small-sized image", attachment=True,
                                help="Small-sized image of asset. It is automatically "
                                     "resized as a 64x64px image, with aspect ratio preserved.")
    mutable_depreciation = fields.Boolean(string="Mutable Depreciation",
                                          readonly=True,
                                          default=lambda self: self.profile_id.mutable_depreciation,
                                          states={'draft': [('readonly', False)]})
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 copy=False, default=lambda self: self.env.user.company_id.id)
    currency_id = fields.Many2one('res.currency', related='company_id.currency_id', store=True, string='Currency')
    _sql_constraints = [
        ('unique_sys_no', 'unique (sys_no)', "System No. Already Exist !"),
    ]

    @api.multi
    def compute_all_depreciation_board(self):
        assets = self.env['account.asset'].search([('type', '=', 'normal')])
        done = []
        for asset in assets:
            asset.compute_depreciation_board()
            done.append(asset)
            _logger.info("completed " + str(len(done))+" Out of "+str(len(assets)),)

    @api.multi
    def compute_all_end_date(self):
        assets = self.env['account.asset'].search([('type', '=', 'normal')])
        done = []
        for asset in assets:
            all_depreciations = sorted(asset.depreciation_line_ids, key=lambda x: x.line_date)
            asset.date_end = all_depreciations[-1].line_date
            done.append(asset)
            _logger.info("completed " + str(len(done))+" Out of "+str(len(assets)),)

    @api.onchange('has_month')
    def change_months(self):
        for rec in self:
            rec.method_number_month = rec.profile_id.method_number_month
    ...
    @api.depends('purchase_value',
                 ...
                 'date_start',
                 'salvage_value')
    def _compute_value_depreciation(self):
        for asset in self:
            if asset.type == 'normal':
                if asset.state != 'draft':
                    asset.value_depreciated = asset.get_accumulated_depreciation(
                        fields.Date.to_string(datetime.today()))
                    asset.value_residual = asset.depreciation_base - asset.value_depreciated
                    asset.value_residual = \
                        asset.depreciation_base - asset.value_depreciated
                else:
                    asset.value_residual = \
                        asset.depreciation_base - asset.value_depreciated
    ...
    def _onchange_profile_id(self):
        ...
        if self.profile_id:
            ...
            self.method_number_month = profile.method_number_month
            self.mutable_depreciation = profile.mutable_depreciation
            self.has_month = profile.has_month
    ...
    def create(self, vals):
        ...
        vals['sys_no'] = self.env['ir.sequence'].next_by_code('account.asset.sys_no')
        ...
        if asset.type == 'normal':
            ...
            asset.method_number_month = asset.profile_id.method_number_month
    ...
    def validate(self):
        for asset in self:
            if asset.type == 'normal' and asset.company_currency_id.is_zero(
                    asset.value_residual):
                asset.state = 'close'
            elif len(asset.depreciation_line_ids) < 2:
                raise UserError("Operation Not Permitted. "
                                "You cannot confirm an asset without computing its depreciation"
                                " Please compute the asset's depreciation first.")
            else:
                asset.state = 'open'
        return True
        ...
        if self.has_month:
            if self.method_time == 'year':
                depreciation_stop_date = depreciation_start_date + \
                    relativedelta(years=self.method_number, months=self.method_number_month+1, days=-1)
            elif self.method_time == 'number':
                if self.method_period == 'month':
                    depreciation_stop_date = depreciation_start_date + \
                        relativedelta(months=self.method_number+self.method_number_month,  days=-1)
                    ...
                    first_line_date = depreciation_start_date \
                        + relativedelta(month=m+self.method_number_month, day=31)
                    months = self.method_number * 3
                    depreciation_stop_date = first_line_date \
                        + relativedelta(months=months+self.method_number_month - 1, days=-1)
                elif self.method_period == 'year':
                    depreciation_stop_date = depreciation_start_date + \
                        relativedelta(years=self.method_number, months=self.method_number_month, days=-1)
    ...
    def _compute_year_amount(self, residual_amount):
        ...
        year_amount_linear = self.depreciation_base / ((self.method_number*12+self.method_number_month)/12.0)
    ...
    def _compute_line_dates(self, table, start_date, stop_date):
        ...
        i = 1
        total_months = self.method_number*12+self.method_number_month
    ...
    def _compute_depreciation_table_lines(self, table, depreciation_start_date,
        ...
        for i, entry in enumerate(table):
            ...
            for li, line_date in enumerate(line_dates):
                ...
                if i == 0 and li == 0:
                    amount = entry.get('period_amount') #self._get_first_period_amount(table, entry, depreciation_start_date, line_dates)
            ...
            if self.method_time == 'year':
                if round(fy_amount_check - fy_amount_check, digits) != 0:
        ...
        for i, entry in enumerate(table):
            ...
            if not entry['fy_amount'] == fy_amount:
        ...
        for i, entry in enumerate(table):
            ...
            if self.method_time == 'year':
                ...
                if self.method_period == 'year':
                    ...
                    total_months = self.method_number * 12 + self.method_number_month
                    period_amount = self.purchase_value/total_months #year_amount / 12
                if i == i_max:
                    if self.method in ['linear-limit', 'degr-limit']:
                ...
                if self.method_time == 'number':
                    number = self.method_number*12+self.method_number_month
                elif self.method_time == 'end':
    ...
    def copy(self):
        ...
        """
        raise UserError('Operation not allowed! Cannot duplicate an Asset. Sys No must be unique')

    @api.multi
    def get_net_book_value(self, date):
        for rec in self:
            if rec.type == 'normal':
                return rec.purchase_value - rec.get_accumulated_depreciation(date)
    ...
    def get_month_depreciation(self, date):
        for rec in self:
            if rec.type == 'normal':
                ...
                total_months = rec.method_number*12+rec.method_number_month
                if fields.Date.from_string(rec.date_end) >= fields.Date.from_string(date):
                    months_covered = diff_month(fields.Date.from_string(date), fields.Date.from_string(rec.date_start))+1
                    month_depreciation = rec.purchase_value/total_months
                return month_depreciation

    @api.multi
    def get_ytd_depreciation(self, date):
        for rec in self:
            if rec.type == 'normal':
                months_covered = 0
                month_depreciation = 0
                ...
                total_months = rec.method_number * 12 + rec.method_number_month
                month_depreciation = rec.purchase_value / total_months
                if fields.Date.from_string(rec.date_end) >= datetime(fields.Date.from_string(date).year, 1, 1).date():
                    if fields.Date.from_string(rec.date_end) >= fields.Date.from_string(date):
                        if fields.Date.from_string(rec.date_start) <= datetime(fields.Date.from_string(date).year, 1, 1).date():
                            months_covered_current_year = diff_month(fields.Date.from_string(date), datetime(fields.Date.from_string(date).year, 1, 1))+1
                        elif fields.Date.from_string(rec.date_start) > datetime(fields.Date.from_string(date).year, 1, 1).date():
                            months_covered_current_year = diff_month(fields.Date.from_string(date), fields.Date.from_string(rec.date_start))+1
                        year_depreciation = months_covered_current_year*month_depreciation
                    if fields.Date.from_string(rec.date_end) < fields.Date.from_string(date):
                        months_covered_current_year = diff_month(fields.Date.from_string(rec.date_end), datetime(fields.Date.from_string(date).year, 1, 1))+1
                        year_depreciation = months_covered_current_year * month_depreciation

                return year_depreciation

    @api.multi
    def get_accumulated_depreciation(self, date):
        for rec in self:
            if rec.type == 'normal':
                months_covered = 0
                month_depreciation = 0
                accum_depr = rec.purchase_value
                total_months = rec.method_number * 12 + rec.method_number_month
                if date and rec.date_end:
                    if fields.Date.from_string(rec.date_end) >= fields.Date.from_string(date):
                        months_covered = diff_month(fields.Date.from_string(date), fields.Date.from_string(rec.date_start))+1
                        month_depreciation = rec.purchase_value / total_months
                        accum_depr = months_covered*month_depreciation
                return accum_depr

    @api.constrains('method_number', 'profile_id', 'method_number_month', 'has_month', 'mutable_depreciation')
    def check_mutation(self):
        for rec in self:
            if not rec.mutable_depreciation:
                if rec.profile_id.method_number != rec.method_number:
                    raise UserError("Asset Depreciation Year must be the same as its Category Depreciation Year")
                if rec.profile_id.method_number_month != rec.method_number_month:
                    raise UserError("Asset Depreciation Months must be "
                                    "the same as its Category Depreciation Months")
                if rec.profile_id.has_month != rec.has_month:
                    raise UserError("If Asset has month then its Category should also has month. Vice Verse")
            if rec.mutable_depreciation:
                if not rec.profile_id.mutable_depreciation:
                    raise UserError("You cannot set this asst's depreciation"
                                    " to be mutable while its category is not mutable. "
