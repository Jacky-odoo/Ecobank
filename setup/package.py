...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Byte  ERP %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('/cygdrive/c/"Program Files"/"Byte  ERP %s"/server/odoo-bin.exe -d mycompany -i base --stop-after-init' % setupversion)
