...
class SyncEvent(object):
    ...
    def compute_OP(self, modeFull=True):
        #If event are already in Gmail and in Byte  ERP
        ...
        # New in Byte  ERP...  Create on create_events of synchronize function
        elif self.OE.found and not self.GG.found:
            if self.OE.status:
                self.OP = Delete('OE', 'Update or delete from GOOGLE')
            else:
                if not modeFull:
                    self.OP = Delete('GG', 'Deleted from Byte  ERP, need to delete it from Gmail if already created')
                else:
                    self.OP = NothingToDo("", "Already Deleted in gmail and unlinked in Byte  ERP")
...
class GoogleCalendar(models.AbstractModel):
    ...
    def create_an_event(self, event):
        """ Create a new event in google calendar from the given event in Byte  ERP.
    ...
    def update_from_google(self, event, single_event_dict, type):
        """ Update an event in Byte  ERP with information from google calendar
    ...
    def update_events(self, lastSync=False):
        ...
        if lastSync:
            ...
            # Select all events from Byte  ERP which have been already synchronized in gmail
