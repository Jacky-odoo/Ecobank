...
class HolidaysType(models.Model):
    ...
    categ_id = fields.Many2one('calendar.event.type', string='Meeting Type',
        help='Once a leave is validated, Byte  ERP will create a corresponding meeting of this type in the calendar.')
...
class Holidays(models.Model):
    ...
    date_to = fields.Datetime('End Date', readonly=True, copy=False, required=True,
        states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]})
    date_resume = fields.Datetime('Resume Date', copy=False, required=True,
        states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]})
    ...
    def _compute_resume_date(self):
        for rec in self:
            if rec.date_to:
                end_date = fields.Datetime.from_string(rec.date_to)
                resume_date = fields.Datetime.to_string(end_date + relativedelta(days=1))
                rec.update({'date_resume': resume_date})
