...
class ControllerREST(http.Controller):
    ...
    def api_auth_gettokens(self):
        ...
        # Odoo login failed:
        if not uid:
            error_descrip = "Messiah User authentication failed!"
            error = 'messiah_user_authentication_failed'
            _logger.error(error_descrip)
            return error_response(402, error, error_descrip)
        
        # GeneraNavigationViewte tokens
        access_token = generate_token()
        expires_in = access_token_expires_in
        refresh_token = generate_token()
        refresh_expires_in = refresh_token_expires_in

        # Keep token against user
