...
class WebRequest(object):
    """ Parent class for all Byte  ERP Web request types, mostly deals with
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Byte  ERP implements token-based `CSRF protection
        ...
        * if the endpoint can be called by external parties (not from
          Byte  ERP) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Byte  ERP Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Byte  ERP Session Expired"
...
Byte  ERP URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.odoo.com/documentation/9.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through Byte  ERP via py-QWeb form, embed a CSRF
...
def db_filter(dbs, httprequest=None):
    ...
    if odoo.tools.config['dbfilter']:
        ...
        # In case --db-filter is not provided and --database is passed, Byte  ERP will
